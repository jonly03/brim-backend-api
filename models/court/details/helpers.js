...
function checkinAnonymous(clientId, courtId) {
	return new Promise((resolve, reject) => {
		...
		// 		}
		// 	})
		console.log(`Anonymously checking clientId/${clientId} into courtId/${courtId}`);
...
function checkoutAnonymousOnDisconnect(clientId) {
	console.log(`Checking clientId/${clientId} out from any court they were checked into due to disconnection`);
	return new Promise((resolve, reject) => {
...
function isClientOnline(clientId) {
	...
	return new Promise((resolve, reject) => {
		mongoDBOnlineRef.find({ client_id: clientId }, (err, doc) => {
			if (err) {
				console.log(`Failed to check if clientId/${clientId} is already online`)
				reject(err);
			}
...
function incrementCourtsNearbyOnlineCounts(clientId, coords) {
	console.log(`Incrementing nearby online counts for courts found near clientId/${clientId}...`)
	return new Promise((resolve, reject) => {
				...
				tryGettingNearbyCourtsMongoDB(coords).then(res => {
					...
					mongoDBCourtsRef.update(
						...
						(err) => {
							...
							console.log(`Done incrementing nearby online counts for courts found near clientId/${clientId}`)
							resolve(courtIds);
						}
					)
				})
...
function decrementCourtsNearbyOnlineCounts(clientId) {
	console.log(`Decrementing nearby online counts for courts found near clientId/${clientId} due to disconnetion`)
	return new Promise((resolve, reject) => {
		...
		mongoDBOnlineRef.findOne(
			...
			(err, doc) => {
				...
				mongoDBOnlineRef.remove({ client_id: clientId }, (err) => {
					if (err) {
						console.log(`Failed to remove clientId/${clientId} from online records`);
						return reject(err);
					}
				...
				tryGettingNearbyCourtsMongoDB(coords).then(res => {
					...
					console.log('Built queries to match all nearby courts with at least 1 online client nearby')
					console.log(queries)
					...
					mongoDBCourtsRef.update(
						...
						(err) => {
							...
							console.log(`Done decrementing nearby online counts for courts found near clientId/${clientId}`)
							resolve(courtIds);
						}
					)
				})
